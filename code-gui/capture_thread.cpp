...
void CaptureThread::getRandomColors(vector<cv::Scalar>& colors, int numColors)
{
    cv::RNG rng(0);
...
void CaptureThread::imageProcessing(cv::Mat &frame)
{
    cv::Mat src = frame.clone();
//    //preprocessing
//    // convert to gray image
//    cv::cvtColor(src, src, cv::COLOR_BGR2HSV);
//    cv::inRange(src, cv::Scalar(0,48,80), cv::Scalar(20,255,255), src);
//    cv::blur(src, src, cv::Size(2,2));
